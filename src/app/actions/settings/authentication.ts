...
import type { AuthFormData } from "@/components/configuration";
 ...
 */
export const updateAuthentication = withAuth(
    async (data: AuthFormData): Promise<Result<AuthFormData>> => {
        try {
            // Read current config
            ...
            const updatedConfig = {
                ...
                authentication: {
                    ...
                    methods: {
                        ...
                        passkey: currentConfig.authentication?.methods
                            ?.passkey || {
                            enabled: false,
                        },
                        // Update GitHub OAuth
 ...
 */
export const getAuthentication = withAuth(
    async (): Promise<Result<AuthFormData>> => {
        try {
            ...
            const authConfig: AuthFormData = {
                github: {
                    enabled:
                        config.authentication.methods.github?.enabled || false,
                    clientId:
                        config.authentication.methods.github?.clientId || "",
                    clientSecret:
                ...
                google: {
                    enabled:
                        config.authentication.methods.google?.enabled || false,
                    clientId:
                        config.authentication.methods.google?.clientId || "",
                    clientSecret:
