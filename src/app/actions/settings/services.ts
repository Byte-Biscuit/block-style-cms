...
import type { ServicesFormData } from "@/components/configuration";
 ...
 */
export const updateServices = withAuth(
    async (data: ServicesFormData): Promise<Result<ServicesFormData>> => {
        try {
            // Read current config
            ...
            if (data.algolia.enabled) {
                if (!data.algolia.appId?.trim()) {
                    return {
                        ...
                        message:
                            "Algolia App ID is required when Algolia is enabled",
                        payload: {} as ServicesFormData,
                    };
                }
...
export const getServices = withAuth(
    async (): Promise<Result<ServicesFormData>> => {
        try {
