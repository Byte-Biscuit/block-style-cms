...
export const getUsers = withAuth(
    async (): Promise<Result<UserWithProvider[]>> => {
        try {
            // Get user list
...
export const getUser = withAuth(
    async (userId: string): Promise<Result<UserWithProvider | null>> => {
        try {
...
export const createUser = withAuth(
    async (data: CreateUserData): Promise<Result<{ userId: string }>> => {
        try {
            // Validate input
            ...
            const emailExists = await UserManagementService.emailExists(
                data.email
            );
            if (emailExists) {
                return {
            ...
            // Create user
            const result =
                await UserManagementService.createCredentialUser(data);
...
export const updateUser = withAuth(
    async (userId: string, data: UpdateUserData): Promise<Result<void>> => {
        try {
 ...
 */
export const resetUserPassword = withAuth(
    async (data: ResetPasswordData): Promise<Result<void>> => {
        try {
            // Validate password length
 ...
 */
export const disableTwoFactor = withAuth(
    async (userId: string): Promise<Result<undefined>> => {
        try {
            // 1. Validate user ID
            if (!userId) {
                return {
 ...
 */
export const generateTwoFactorSecret = withAuth(
    async (
        userId: string
    ): Promise<
        Result<{
    ...
    > => {
        try {
            ...
            const result =
                await UserManagementService.generateTwoFactorSecret(userId);
            ...
            console.error(
                "[UserManagement] Error generating 2FA secret:",
                error
            );
            return {
                code: HttpStatus.INTERNAL_SERVER_ERROR,
 ...
 * @returns Success or error result
 */
export const verifyAndEnableTwoFactor = withAuth(
    async (userId: string, code: string): Promise<Result<undefined>> => {
        try {
            // 1. Validate inputs
            if (!userId || !code) {
                return {
            ...
            const isValid =
                await UserManagementService.verifyAndEnableTwoFactor(
                    userId,
                    code
                );
