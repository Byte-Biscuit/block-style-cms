import { getLanguageDisplayName, localeMap } from "@/i18n/config";
...
export const POST = withTiming(async (request: Request) => {
    ...
    if (needsLanguage && (!language || !(language in localeMap))) {
        return failure(
            "The language parameter is required and must be one of the supported languages"
        );
    }
    ...
    const taskHandlers = {
        slug: async () => {
            ...
            while (
                metadataMap[finalSlug] &&
                metadataMap[finalSlug].length > 0
            ) {
                counter++;
        ...
        summary: async () => {
            ...
            return success("Summary generated successfully", {
                summary: result,
            });
        },
        keywords: async () => {
            ...
            return success("Keywords generated successfully", {
                keywords: result,
            });
        },
        suggestion: async () => {
            ...
            if (!result) {
                return failure(
                    "Suggestion generation failed, please try again"
                );
            }
            return success("Suggestion generated successfully", {
                suggestion: result,
            });
        },
        batch: async () => {
            ...
            if (!result) {
                return failure(
                    "Batch generation failed, please try again or use single generation"
                );
            }
    ...
    return failure(
        "The provided genType parameter is not recognized; please check your request"
    );
});
