...
export const GET = withTiming(async (request: NextRequest) => {
    try {
        ...
        const { tag, locale, published, searchTerm, pageIndex, pageSize } =
        ...
        const articles = await articleService.getArticlesWithPage(
            targetArticles,
            pageIndex,
            pageSize
        );
        const pageData = {
            ...
            totalPages: await articleService.getTotalPageCount(
                targetArticles,
                pageSize
            ),
        };
...
export const POST = withTiming(async (request) => {
    try {
        ...
        const schema = requestBody.published
            ? articleSchema
            : draftArticleSchema;
...
export const PUT = withTiming(async (request) => {
    try {
        ...
        const locale = requestBody[LOCALE_PARAM_NAME] || defaultLocale;
        const t = await getTranslations({ locale: locale });
        const { updateArticleSchema, draftUpdateArticleSchema } =
