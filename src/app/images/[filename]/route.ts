...
export async function GET(
    ...
    try {
        ...
        if (
            fit &&
            ["cover", "contain", "fill", "inside", "outside"].includes(fit)
        ) {
            options.fit = fit as
                | "cover"
                | "contain"
        ...
        const needsProcessing =
            options.w || options.h || options.q || options.f || options.fit;
        ...
        if (
            ifNoneMatch &&
            ifNoneMatch.includes(`"${imageInfo.filename}-${imageInfo.size}"`)
        ) {
