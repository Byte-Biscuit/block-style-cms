...
export const ColorPickerButton = ({ icon, type }: ColorPickerProps) => {
    ...
    const tooltipText =
        type === "textColor"
            ? t("formatting_toolbar.text_color", "Text Color")
            : t("formatting_toolbar.background_color", "Background Color");
    ...
    const menuContent = (
        ...
        >
            {BLOCKNOTE_COLORS.map((color) => {
                const displayName = t(
                    `color_picker.colors.${color.value}`,
                    color.name
                );
                return (
                    <button
                    ...
                    >
                        ...
                        >
                            {type === "textColor" && (
                                <span
                                    className={`flex justify-center items-center text-[10px] font-bold ${color.value === "default" ? "text-white" : ""}`}
                                    style={{
                                        color:
                                            color.value !== "default"
                                                ? "inherit"
                        ...
                        <span
                            className={`text-gray-700 dark:text-gray-200 ${type === "textColor" ? color.textClass : ""}`}
                        >
                            {displayName}
                        </span>
                    </button>
