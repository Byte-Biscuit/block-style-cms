 ...
 */
async function getAllUsers(): Promise<UserWithProvider[]> {
    try {
        ...
        console.error(
            "[UserManagementService] Error getting all users:",
            error
        );
        throw error;
    }
 ...
 */
async function getUserById(userId: string): Promise<UserWithProvider | null> {
    try {
        const auth = await getAuth();
        const db = auth.options.database as any;
        ...
        console.error(
            "[UserManagementService] Error getting user by ID:",
            error
        );
        throw error;
    }
 ...
 */
async function getCredentialUsers(): Promise<UserWithProvider[]> {
    try {
        ...
        console.error(
            "[UserManagementService] Error getting credential users:",
            error
        );
        throw error;
    }
...
async function createCredentialUser(
    ...
    try {
        ...
        // 1.7: issuer + accountId is the identity key; credential accountId is user.id
        db.prepare(
            `
            INSERT INTO account (id, userId, providerId, accountId, issuer, password, createdAt, updatedAt)
            VALUES (?, ?, 'credential', ?, 'local:credential', ?, ?, ?)
        `
        ).run(accountRowId, userId, userId, hashedPassword, now, now);

        console.log(
            `[UserManagementService] User created: ${userId} (${data.email})`
        );
...
async function updateUserName(userId: string, name: string): Promise<void> {
    try {
        ...
        console.error(
            "[UserManagementService] Error updating user name:",
            error
        );
        throw error;
    }
...
async function updateUserPassword(
    ...
    try {
        ...
        console.error(
            "[UserManagementService] Error updating password:",
            error
        );
        throw error;
    }
...
async function deleteUser(userId: string): Promise<void> {
    try {
        ...
        // 1. Delete API keys owned by this user (plugin table)
        const apikeyTable = db
            .prepare(
                `SELECT 1 AS ok FROM sqlite_master WHERE type='table' AND name='apikey'`
            )
            .get();
        if (apikeyTable) {
            db.prepare("DELETE FROM apikey WHERE referenceId = ?").run(userId);
        }

        // 2. Delete all associated accounts
...
async function verifyPassword(
    ...
    try {
        ...
        console.error(
            "[UserManagementService] Error verifying password:",
            error
        );
        return false;
    }
...
async function getUserCount(): Promise<number> {
    try {
        ...
        console.error(
            "[UserManagementService] Error getting user count:",
            error
        );
        throw error;
    }
...
async function isTwoFactorEnabled(userId: string): Promise<boolean> {
    try {
        ...
        console.error(
            "[UserManagementService] Error checking 2FA status:",
            error
        );
        return false;
    }
 ...
 */
async function disableTwoFactor(userId: string): Promise<void> {
    try {
        const auth = await getAuth();
        const db = auth.options.database as any;
        ...
        db.prepare("UPDATE user SET twoFactorEnabled = 0 WHERE id = ?").run(
            userId
        );
...
function generateBackupCodesFn(options?: BackupCodeOptions | undefined) {
    return Array.from({ length: options?.amount ?? 10 })
        .fill(null)
        .map(() =>
            generateRandomString(options?.length ?? 10, "a-z", "0-9", "A-Z")
        )
...
async function generateBackupCodes(
    ...
    const storeBackupCodesOption = options?.storeBackupCodes;
    if (
        storeBackupCodesOption &&
        typeof storeBackupCodesOption === "object" &&
        "encrypt" in storeBackupCodesOption
    ) {
        return {
            backupCodes,
            encryptedBackupCodes: await storeBackupCodesOption.encrypt(
 ...
 */
async function generateTwoFactorSecret(userId: string): Promise<{
    secret: string;
    otpauthUrl: string;
    backupCodes: string[];
}> {
    try {
        ...
        const twoFactorPlugin = auth.options.plugins?.find(
            (p: any) => p.id === "two-factor"
        ) as TwoFactorPluginWithOptions | undefined;
        const storeBackupCodes =
            twoFactorPlugin?.options?.backupCodeOptions?.storeBackupCodes ||
            "plain";
        console.log(
            "[UserManagementService] storeBackupCodes option:",
            storeBackupCodes
        );
        const backupCodes = await UserManagementService.generateBackupCodes(
            authSecret,
            { storeBackupCodes }
        );
        ...
        console.log(
            `[UserManagementService] 2FA secret generated for user: ${userId}`
        );
        ...
        console.error(
            "[UserManagementService] Error generating 2FA secret:",
            error
        );
        throw error;
    }
...
async function verifyAndEnableTwoFactor(
    ...
    try {
        ...
        console.log(
            "[2FA Debug] Decrypted secret length:",
            decryptedSecret.length
        );
        ...
        db.prepare("UPDATE user SET twoFactorEnabled = 1 WHERE id = ?").run(
            userId
        );
        ...
        console.error(
            "[UserManagementService] Error verifying 2FA code:",
            error
        );
        throw error;
    }
