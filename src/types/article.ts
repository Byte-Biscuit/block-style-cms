...
export function createArticleSchemas(t: TranslationFunction) {
    ...
    const localeSchema = z.enum(
        localeCodesTmp,
        t("admin.validation.article.locale.invalid", {
            codes: localeCodesTmp.join(", "),
        })
    );
    ...
    const imageSchema = z
        .url(t("admin.validation.article.image.url"))
        .optional()
        .or(z.literal(""));
...
export type Article = z.infer<
    ReturnType<typeof createArticleSchemas>["articleSchema"]
>;
export type UpdateArticle = z.infer<
    ReturnType<typeof createArticleSchemas>["updateArticleSchema"]
>;
export type ArticleMetadata = z.infer<
    ReturnType<typeof createArticleSchemas>["articleMetadataSchema"]
>;
